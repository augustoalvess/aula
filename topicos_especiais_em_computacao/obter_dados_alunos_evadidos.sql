...
CANCELAMENTO
TRANCAMENTO
PRÉ-MATRICULA CANCELADA
TRANSFERÊNCIA EXTERNA PARA
...
contratos AS (
	...
	INNER JOIN acdContract
     ...
     WHERE mov.stateTime = mov.maxStateTime
       AND mov.inOutTransition = 'O'
       AND mov.stateContractId IN (5, 7, 12, 13, 16, 24, 30)
     LIMIT 100
),
...
INNER JOIN ONLY basPhysicalPerson
      ...
      LEFT JOIN academico
	     ON academico.contractId = contratos.contractId) TO '/home/augusto/Documentos/aula/topicos_especiais_em_computacao/dados_alunos_evadidos.csv' WITH CSV;
